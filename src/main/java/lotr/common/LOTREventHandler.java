...
public class LOTREventHandler implements IFuelHandler {
    ...
    public void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if(!world.isRemote) {
            ...
            LOTRLevelData.getData(entityplayermp).send35AlignmentChoice(entityplayermp, world);
        }
    }
        ...
        if(!world.isRemote && entityplayer instanceof EntityPlayerMP && world instanceof WorldServer) {
            ...
            if(deathDimension == LOTRDimension.MIDDLE_EARTH.dimensionID) {
                if(LOTRConfig.middleEarthRespawning) {
                    ...
                    if(deathPoint != null) {
                        boolean flag;
                        boolean bl2 = flag = deathPoint.getDistanceSquaredToChunkCoordinates(spawnLocation) > respawnThreshold * respawnThreshold;
                        if(flag) {
    ...
    public void onBlockInteract(PlayerInteractEvent event) {
        ...
        if(event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
            ...
            if(block == Blocks.ender_chest && LOTRDimension.getCurrentDimension(world) == LOTRDimension.SHADOW && LOTRConfig.disableEnderChestsUtumno) {
                event.setCanceled(true);
                return;
            }
        ...
        if(event.action == PlayerInteractEvent.Action.LEFT_CLICK_BLOCK) {
            ...
            if(!world.isRemote && LOTRBannerProtection.isProtectedByBanner(world, i1, j1, k1, LOTRBannerProtection.forPlayer(entityplayer), true) && block1 instanceof BlockFire) {
                event.setCanceled(true);
                return;
            }
        }
    ...
    public void onBlockBreaking(BreakSpeed event) {
        ...
        int j = event.y;
        int k = event.z;
        if(block instanceof LOTRWorldProviderUtumno.UtumnoBlock && LOTRDimension.getCurrentDimension(world) == LOTRDimension.SHADOW) {
    ...
    public void onUseBonemeal(BonemealEvent event) {
        ...
        if(!world.isRemote) {
            ...
            if(event.block instanceof LOTRBlockSaplingBase) {
                ...
                int meta2 = world.getBlockMetadata(i, j, k);
                if(rand.nextFloat() < 0.45) {
                    sapling.incrementGrowth(world, i, j, k, rand);
                }
                if(sapling == LOTRMod.sapling4 && (meta2 & 7) == 1 && world.getBlock(i, j, k) == LOTRMod.wood4 && world.getBlockMetadata(i, j, k) == 1) {
    ...
    public void onEntitySpawnAttempt(LivingSpawnEvent.CheckSpawn event) {
        ...
        if(!world.isRemote && entity instanceof EntityMob && LOTRBannerProtection.isProtectedByBanner(world, entity, LOTRBannerProtection.anyBanner(), false)) {
            event.setResult(Event.Result.DENY);
            return;
        }
    }
    ...
    public void onEntityJoinWorld(EntityJoinWorldEvent event) {
        ...
        if(!world.isRemote && entity instanceof EntityCreature && (obj = LOTREntityRegistry.registeredNPCs.get(s = EntityList.getEntityString(entitycreature = (EntityCreature) entity))) != null) {
    ...
    public void onLivingUpdate(LivingEvent.LivingUpdateEvent event) {
        ...
        if(!world.isRemote && LOTRMod.canSpawnMobs(world) && entity.isEntityAlive() && entity.isInWater() && entity.ridingEntity == null) {
            boolean flag = true;
            ...
            if(flag) {
                ...
                int j2 = world.getTopSolidOrLiquidBlock(i, k3);
                while(world.getBlock(i, j2 + 1, k3).getMaterial().isLiquid() || world.getBlock(i, j2 + 1, k3).getMaterial().isSolid()) {
                    ++j2;
                }
                if(j2 - entity.boundingBox.minY < 2.0 && world.getBlock(i, j2, k3).getMaterial() == Material.water && world.getBiomeGenForCoords(i, k3) instanceof LOTRBiomeGenDeadMarshes) {
                    ...
                    if(!anyNearbyWraiths) {
                        ...
                        int k1 = k3 + MathHelper.getRandomIntegerInRange(world.rand, -3, 3);
        ...
        if(!world.isRemote && LOTRMod.canSpawnMobs(world) && entity.isEntityAlive() && world.isDaytime()) {
            ...
            if(LOTRFaction.HOBBIT.isBadRelation(LOTRMod.getNPCFaction(entity))) {
                ...
                f = health * 2.5f;
                i2 = (int) (health / 15.0f);
                shirriffs = 2 + world.rand.nextInt(i2 + 1);
            }
            ...
            if(f > 0.0f) {
                ...
                i2 = MathHelper.floor_double(entity.posX);
                int k4 = MathHelper.floor_double(entity.posZ);
                int j3 = world.getTopSolidOrLiquidBlock(i2, k4);
                if(world.rand.nextInt(chance) == 0 && world.getBiomeGenForCoords(i2, k4) instanceof LOTRBiomeGenShire && (nearbyShirriffs = world.getEntitiesWithinAABB(LOTREntityHobbitShirriff.class, entity.boundingBox.expand(12.0, 6.0, 12.0))).isEmpty()) {
                    boolean sentMessage = false;
                    boolean playedHorn = false;
                    block3: for(int l4 = 0; l4 < shirriffs; ++l4) {
                        LOTREntityHobbitShirriff shirriff = new LOTREntityHobbitShirriff(world);
                        for(int l1 = 0; l1 < 32; ++l1) {
                            int j1;
                            int k1;
                            int i1 = i2 - world.rand.nextInt(12) + world.rand.nextInt(12);
                            if(!world.getBlock(i1, (j1 = world.getTopSolidOrLiquidBlock(i1, k1 = k4 - world.rand.nextInt(12) + world.rand.nextInt(12))) - 1, k1).isSideSolid(world, i1, j1 - 1, k1, ForgeDirection.UP) || world.getBlock(i1, j1, k1).isNormalCube() || world.getBlock(i1, j1 + 1, k1).isNormalCube()) continue;
                            shirriff.setLocationAndAngles(i1 + 0.5, j1, k1 + 0.5, 0.0f, 0.0f);
                            if(!shirriff.getCanSpawnHere() || !(entity.getDistanceToEntity(shirriff) > 6.0)) continue;
                            ...
                            if(!sentMessage && entity instanceof EntityPlayer) {
                                EntityPlayer entityplayer = (EntityPlayer) entity;
                                shirriff.sendSpeechBank(entityplayer, shirriff.getSpeechBank(entityplayer));
                                sentMessage = true;
                            }
                            if(playedHorn) continue block3;
                            world.playSoundAtEntity(shirriff, "lotr:item.horn", 2.0f, 2.0f);
                            playedHorn = true;
                            continue block3;
                        }
                    }
        ...
        if(!world.isRemote && entity.isEntityAlive() && entity.isInWater() && entity.ridingEntity == null && entity.ticksExisted % 10 == 0) {
            ...
            if(flag && (biome = world.getBiomeGenForCoords(i = MathHelper.floor_double(entity.posX), k = MathHelper.floor_double(entity.posZ))) instanceof LOTRBiomeGenMirkwoodCorrupted) {
        ...
        if(!world.isRemote && entity.isEntityAlive() && entity.isInWater() && entity.ridingEntity == null && entity.ticksExisted % 10 == 0) {
            ...
            if(flag && (biome = world.getBiomeGenForCoords(i = MathHelper.floor_double(entity.posX), k = MathHelper.floor_double(entity.posZ))) instanceof LOTRBiomeGenMorgulVale) {
                entity.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 600, 1));
                entity.addPotionEffect(new PotionEffect(Potion.digSlowdown.id, 600, 1));
                entity.addPotionEffect(new PotionEffect(Potion.weakness.id, 600));
        ...
        if(!world.isRemote && entity.isEntityAlive() && entity.ticksExisted % 20 == 0) {
            ...
            if(flag) {
                ...
                if(biome instanceof LOTRBiomeGenForodwaith && (world.canBlockSeeTheSky(i, j, k2) || entity.isInWater()) && world.getSavedLightValue(EnumSkyBlock.Block, i, j, k2) < 10) {
                    int frostChance = 50;
        ...
        if(!world.isRemote && entity.isEntityAlive() && entity.ticksExisted % 20 == 0) {
            ...
            if(flag) {
                ...
                k2 = MathHelper.floor_double(entity.posZ);
                BiomeGenBase biome = world.getBiomeGenForCoords(i, k2);
                if(biome instanceof LOTRBiomeGenNearHarad && !entity.isInWater() && world.canBlockSeeTheSky(i, j, k2) && world.isDaytime()) {
                    boolean attacked;
                    int burnChance = 50;
    ...
    public void onEntityInteract(EntityInteractEvent event) {
        ...
        ItemStack itemstack = entityplayer.inventory.getCurrentItem();
        Entity entity = event.target;
        if(!world.isRemote && (entity instanceof EntityHanging || entity instanceof LOTRBannerProtectable) && LOTRBannerProtection.isProtectedByBanner(world, entity, LOTRBannerProtection.forPlayer(entityplayer), true)) {
            event.setCanceled(true);
            return;
        }
    ...
    public void onLivingAttacked(LivingAttackEvent event) {
        ...
        if(event.source instanceof EntityDamageSourceIndirect) {
            ...
            if(!world.isRemote && entity instanceof EntityPlayer &&
            	attacker instanceof LOTREntityOrc && projectile instanceof LOTREntitySpear && projectile instanceof LOTREntityJavelin &&
            	(chestplate = entity.getEquipmentInSlot(3)) != null && chestplate.getItem() == LOTRMod.bodyMithril) {
    ...
    public void onLivingDeath(LivingDeathEvent event) {
        ...
        if(!world.isRemote) {
            ...
            if(source.getEntity() instanceof EntityPlayer) {
                ...
                if(npc.hiredNPCInfo.isActive && npc.hiredNPCInfo.getHiringPlayer() != null) {
                    ...
                    boolean bl = byNearbyUnit = npc.getDistanceSqToEntity(entityplayer) <= nearbyDist * nearbyDist;
                }
            }
        ...
        if(!world.isRemote) {
            ...
            if(attackingPlayer != null) {
                boolean isFoe;
                boolean bl = isFoe = LOTRLevelData.getData(attackingPlayer).getAlignment(LOTRMod.getNPCFaction(entity)) < 0.0f;
                if(isFoe) {
        ...
        if(!world.isRemote && exploder != null) {
            ...
            if(protectFilter != null) {
                ...
                for(ChunkPosition blockPos : blockList) {
                    ...
                    if(!LOTRBannerProtection.isProtectedByBanner(world, i, j, k, protectFilter, false)) continue;
    ...
    public void onServerChat(ServerChatEvent event) {
        ...
        if(MinecraftServer.getServer().isDedicatedServer() && !entityplayer.capabilities.isCreativeMode && !LOTRLevelData.getData(entityplayer).getAskedForGandalf() && StringUtils.containsIgnoreCase(message, "add gandalf")) {
            boolean success = false;
            LOTRInvasions[] invasions = LOTRInvasions.values();
            List<LOTRInvasions> invasionsAsList = Arrays.asList(invasions);
            Collections.shuffle(invasionsAsList);
            invasions = invasionsAsList.toArray(invasions);
            for(LOTRInvasions invasion : invasions) {
                if(invasion.invasionMobs.isEmpty() || LOTRLevelData.getData(entityplayer).getAlignment(invasion.invasionFaction) >= 0.0f) continue;
                LOTREntityInvasionSpawner invasionSpawner = new LOTREntityInvasionSpawner(entityplayer.worldObj);
                invasionSpawner.setInvasionType(invasion);
                double x = entityplayer.posX;
                double y = entityplayer.boundingBox.minY + 3.0 + entityplayer.getRNG().nextDouble() * 2.0;
                double z = entityplayer.posZ;
                invasionSpawner.setLocationAndAngles(x, y, z, 0.0f, 0.0f);
                if(!invasionSpawner.canInvasionSpawnHere()) continue;
                entityplayer.worldObj.spawnEntityInWorld(invasionSpawner);
