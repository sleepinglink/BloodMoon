...
public abstract class LOTRMiniQuest {
    ...
    public void start(EntityPlayer entityplayer, LOTREntityNPC npc) {
        ...
        BiomeGenBase biome = entityplayer.worldObj.getBiomeGenForCoords(i, k = MathHelper.floor_double(entityplayer.posZ));
    ...
    protected void complete(EntityPlayer entityplayer, LOTREntityNPC npc) {
        ...
        if(alignment != 0.0f) {
            ...
            LOTRAlignmentBonusMap alignmentMap = this.playerData.addAlignment(entityplayer, bonus, this.entityFaction, npc);
            this.alignmentRewarded = alignmentMap.get((this.entityFaction)).floatValue();
        }
        ...
        if(!dropItems.isEmpty()) {
            boolean givePouch;
            boolean bl = givePouch = rand.nextInt(10) == 0;
            if(givePouch) {
    ...
    public void updateLocation(LOTREntityNPC npc) {
        ...
        if(prevCoords == null) {
            ...
            boolean bl = sendUpdate = coords.getDistanceSquaredToChunkCoordinates(prevCoords) > 256.0;
        }
        if(sendUpdate) {
            this.updateQuest();
        }
    ...
    public ChunkCoordinates getLastLocation() {
        return this.lastLocation == null ? null : this.lastLocation.getLeft();
    }
