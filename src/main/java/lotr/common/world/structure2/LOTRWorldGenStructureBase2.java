...
public abstract class LOTRWorldGenStructureBase2 extends WorldGenerator {
    ...
    public LOTRChestContents chestContent = null;
    private StructureBoundingBox sbb;
    private LOTRStructureScan currentStrScan;
    private Map<String, BlockAliasPool> scanAliases = new HashMap<String, BlockAliasPool>();
    private Map<String, Float> scanAliasChances = new HashMap<String, Float>();

    public LOTRWorldGenStructureBase2(boolean flag) {
        super(flag);
        this.notifyChanges = flag;
    }
    public void setChestContent(LOTRChestContents chestContents) {
        this.chestContent = chestContents;
     }
    @Override
    ...
    public void clearStructureField(World world, int width, int height) {
        for(int i = -width; i <= width; ++i) {
           for(int k = -height; k <= height; ++k) {
              for(int j = 21; j != 0; --j) {
                 Block block = this.getBlock(world, i, j, k);
                 this.setAir(world, i, j, k);
                 if (block != Blocks.air) {
                    this.setBlockAndMetadata(world, i, j - 1, k, block, 0);
                 }
              }
           }
        }
    ...
    protected void placeFlowerPot(World world, int i, int j, int k, ItemStack itemstack) {
        boolean vanilla;
        boolean bl = vanilla = itemstack == null || itemstack.getItem() == Item.getItemFromBlock(Blocks.cactus);
        if(vanilla) {
    ...
    protected ItemStack getRandomFlower(World world, Random random) {
        ...
        if(biome instanceof LOTRBiome) {
            BiomeGenBase.FlowerEntry fe = ((LOTRBiome) biome).getRandomFlower(random);
    ...
    protected void setBlockAliasChance(String alias, float chance) {
        this.scanAliasChances.put(alias, Float.valueOf(chance));
    }
    ...
    protected final boolean isSurface(World world, int i, int j, int k) {
        ...
        return this.villageInstance != null && this.villageInstance.isVillageSurface(world, i, j, k);
    }
