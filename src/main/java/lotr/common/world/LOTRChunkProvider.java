    ...
    public LOTRChunkProvider(World world, long seed) {
        ...
        for(int i = -this.biomeSampleRadius; i <= this.biomeSampleRadius; ++i) {
            for(int k = -this.biomeSampleRadius; k <= this.biomeSampleRadius; ++k) {
                float f;
                this.biomeHeightNoise[i + this.biomeSampleRadius + (k + this.biomeSampleRadius) * this.biomeSampleWidth] = f = 10.0f / MathHelper.sqrt_float(i * i + k * k + 0.2f);
            }
        }
    }
        ...
        for(int i1 = 0; i1 < xSize; ++i1) {
            for(int k1 = 0; k1 < zSize; ++k1) {
                ...
                for(int j1 = 0; j1 < ySize; ++j1) {
                    ...
                    baseHeight += heightNoise * 0.2 * centreVariant.hillFactor;
                    ...
                    double var38 = (this.noise3[noiseIndex] / 10.0 + 1.0) / 2.0 * centreVariant.hillFactor;
