...
public class LOTRWorldGenMangrove extends WorldGenAbstractTree {
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        if(j >= 1 && j + height + 1 <= 256) {
            ...
            boolean bl = canGrow = below.canSustainPlant(world, i, j - 1, k, ForgeDirection.UP, (IPlantable) (Blocks.sapling)) || below.canSustainPlant(world, i, j - 1, k, ForgeDirection.UP, Blocks.deadbush);
            if(canGrow) {
                int j1;
                ...
                for(j1 = j - leafStart + height; j1 <= j + height; ++j1) {
                    ...
                    for(int i1 = i - leafRange; i1 <= i + leafRange; ++i1) {
                        ...
                        for(int k1 = k - leafRange; k1 <= k + leafRange; ++k1) {
                            ...
                            if(Math.abs(i2) == leafRange && Math.abs(k2) == leafRange && (random.nextInt(2) == 0 || j2 == 0) || !block.canBeReplacedByLeaves(world, i1, j1, k1)) continue;
                            ...
                            if(random.nextInt(8) != 0 || !world.getBlock(i1, j1, k1 + 1).isAir(world, i1, j1, k1 + 1)) continue;
                ...
                for(j1 = 0; j1 < height; ++j1) {
                    ...
                    if(!block.isReplaceable(world, i, j + j1, k) && !block.isLeaves(world, i, j + j1, k)) continue;
