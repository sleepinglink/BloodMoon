...
public class LOTRWorldGenWillow extends WorldGenAbstractTree {
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        if(!(isSoil = (below = world.getBlock(i, j - 1, k)).canSustainPlant(world, i, j - 1, k, ForgeDirection.UP, (IPlantable) (Blocks.sapling)))) {
            flag = false;
        }
        if(this.needsWater) {
            boolean water = false;
            ...
            for(int l = 0; l < attempts; ++l) {
                ...
                if(world.getBlock(i1, j1 = j + MathHelper.getRandomIntegerInRange(random, -8, 4), k1 = k + MathHelper.getRandomIntegerInRange(random, -12, 12)).getMaterial() != Material.water) continue;
                water = true;
                break;
            }
            if(!water) {
