...
public class LOTRItemBow extends ItemBow {
    ...
    public static void applyBowModifiers(EntityArrow arrow, ItemStack itemstack) {
        ...
        if((fireAspect = EnchantmentHelper.getEnchantmentLevel(Enchantment.flame.effectId, itemstack) + LOTREnchantmentHelper.calcFireAspect(itemstack)) > 0) {
            arrow.setFire(100);
        }
    ...
    private int getInvArrowSlot(EntityPlayer entityplayer) {
        for(int slot = 0; slot < entityplayer.inventory.mainInventory.length; ++slot) {
            ...
            if(invItem == null || invItem.getItem() != Items.arrow && invItem.getItem() != LOTRMod.arrowPoisoned) continue;
            return slot;
        }
        return -1;
    }
    ...
    public IIcon getIcon(ItemStack itemstack, int renderPass, EntityPlayer entityplayer, ItemStack usingItem, int useRemaining) {
        BowState bowState = this.getBowState(entityplayer, usingItem, useRemaining);
    ...
    public void registerIcons(IIconRegister iconregister) {
        ...
        if(bowRenderer instanceof LOTRRenderBow && ((LOTRRenderBow) bowRenderer).isLargeBow()) {
            Arrays.fill(this.bowPullIcons, this.itemIcon);
        }
        else {
