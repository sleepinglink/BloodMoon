...
public class LOTREntityAIFarm extends EntityAIBase {
  ...
  public void updateTask() {
    ...
    if (!canDoAction) {
      ...
      if (canBonemeal) {
        this.theEntity.swingItem();
        ItemStack bonemeal = getInventoryBonemeal();
        if (ItemDye.applyBonemeal(getInventoryBonemeal(), this.theWorld, this.actionTarget.posX, this.actionTarget.posY, this.actionTarget.posZ, (EntityPlayer)this.fakePlayer))
  ...
  public List<TileEntityChest> gatherNearbyChests() {
    ...
    int y = MathHelper.floor_double(this.theEntity.boundingBox.minY);
    ...
    List<TileEntityChest> nearbyChests = new ArrayList<TileEntityChest>();
  ...
  public ChunkCoordinates getAdjacentSolidOpenWalkTarget(int i, int j, int k) {
    List<ChunkCoordinates> possibleCoords = new ArrayList<ChunkCoordinates>();
