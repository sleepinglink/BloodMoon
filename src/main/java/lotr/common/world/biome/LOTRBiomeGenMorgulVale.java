...
public class LOTRBiomeGenMorgulVale extends LOTRBiomeGenMordor {
    ...
    public void generateBiomeTerrain(World world, Random random, Block[] blocks, byte[] meta, int i, int k, double stoneNoise, int height, LOTRBiomeVariant variant) {
        ...
        if(d5 + (d6 = this.noiseRock.func_151601_a(i * 0.3, k * 0.3)) > 1.1) {
    ...
    public void decorate(World world, Random random, int i, int k) {
        ...
        for(int l = 0; l < 4; ++l) {
            ...
            for(int a = 0; a < 20; ++a) {
                ...
                Block block = world.getBlock(i2, j2 = j1 + MathHelper.getRandomIntegerInRange(random, -range, range), k2 = k1 + MathHelper.getRandomIntegerInRange(random, -range, range));
