...
public class LOTRBiomeVariantScrubland extends LOTRBiomeVariant {
    ...
    public void generateVariantTerrain(World world, Random random, Block[] blocks, byte[] meta, int i, int k, int height, LOTRBiome biome) {
        ...
        if(d1 + (d2 = LOTRBiome.biomeTerrainNoise.func_151601_a(i * 0.07, k * 0.07)) + (d3 = LOTRBiome.biomeTerrainNoise.func_151601_a(i * 0.3, k * 0.3)) > 0.6) {
            j = height;
        ...
        if((d1 = LOTRBiome.biomeTerrainNoise.func_151601_a(i * 0.008, k * 0.008)) + (d2 = LOTRBiome.biomeTerrainNoise.func_151601_a(i * 0.05, k * 0.05)) + (d3 = LOTRBiome.biomeTerrainNoise.func_151601_a(i * 0.6, k * 0.6)) > 0.8 && random.nextInt(3) == 0) {
            j = height;
            index = xzIndex * ySize + j;
