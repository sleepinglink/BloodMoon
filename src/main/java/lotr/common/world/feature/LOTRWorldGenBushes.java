        ...
        if(below.canSustainPlant(world, i, j - 1, k, ForgeDirection.UP, (IPlantable) (Blocks.sapling))) {
            ...
            for(int i1 = -size; i1 <= size; ++i1) {
                for(k1 = -size; k1 <= size; ++k1) {
                    ...
                    int i2 = i + i1;
                    int k2 = k + k1;
                    if(size != 0 && Math.abs(i1) == size && Math.abs(k1) == size && random.nextInt(3) != 0 || (block = world.getBlock(i2, j, k2)).getMaterial().isLiquid() || !block.isReplaceable(world, i2, j, k2)) continue;
