...
public class LOTRWorldGenNearHaradDesertCamp extends LOTRWorldGenStructureBase2 {
    ...
    public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
        ...
        boolean bl = flag = new LOTRWorldGenNearHaradTent(this.notifyChanges).generateWithSetRotation(world, random, this.getX(x, z), this.getY(y), this.getZ(x, z), (this.getRotationMode() + 2) % 4) || flag;
        if(!flag) {
            return false;
        }
        for(int i1 = -1; i1 <= 1; ++i1) {
