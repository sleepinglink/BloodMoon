        ...
        if(j >= 1 && j + height + 1 <= 256) {
                ...
                for(int i1 = i - range; i1 <= i + range && flag; ++i1) {
                    for(int k1 = k - range; k1 <= k + range && flag; ++k1) {
                        if(j1 >= 0 && j1 < 256) {
                            ...
                            if(block.isAir(world, i1, j1, k1) || block.isLeaves(world, i1, j1, k1)) continue;
                            flag = false;
                            continue;
                        }
                        flag = false;
            ...
            if(isSoil && j < 256 - height - 1) {
                ...
                for(int leafLayer = 0; leafLayer <= leafStart; ++leafLayer) {
                    ...
                    for(int i1 = i - leafRange; i1 <= i + leafRange; ++i1) {
                        ...
                        for(int k1 = k - leafRange; k1 <= k + leafRange; ++k1) {
                            int k2 = k1 - k;
                            if(Math.abs(i2) == leafRange && Math.abs(k2) == leafRange && leafRange > 0 || !world.getBlock(i1, j1, k1).canBeReplacedByLeaves(world, i1, j1, k1)) continue;
                ...
                for(j1 = 0; j1 < height - trunkTop; ++j1) {
                    ...
                    if(!block2.isAir(world, i, j + j1, k) && !block2.isLeaves(world, i, j + j1, k)) continue;
