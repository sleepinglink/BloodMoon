...
import lotr.common.entity.npc.LOTREntityNumenorFarmer;
import lotr.common.entity.npc.LOTREntityNumenorFarmhand;
...
public class LOTRWorldGenNumenorBarn extends LOTRWorldGenNumenorStructure {
    public LOTRWorldGenNumenorBarn(boolean flag) {
        super(flag);
    }
    ...
    public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
        ...
        this.placeChest(world, random, 3, 6, 11, 5, LOTRChestContents2.NUMENOR_HOUSE);
        for(i12 = -3; i12 <= -2; ++i12) {
            for(k1 = 7; k1 <= 8; ++k1) {
        ...
        if(random.nextInt(3) == 0) {
            if(random.nextBoolean()) {
                this.placeChest(world, random, -2, 6, 3, 4, LOTRChestContents2.NUMENOR_HOUSE_TREASURE);
            }
            else {
                this.placeChest(world, random, -1, 6, 11, 4, LOTRChestContents2.NUMENOR_HOUSE_TREASURE);
            }
        }
        for(i12 = -4; i12 <= 4; ++i12) {
            for(k1 = 0; k1 <= 13; ++k1) {
        ...
        for(int l = 0; l < animals; ++l) {
            EntityAnimal animal = LOTRWorldGenNumenorBarn.getRandomAnimal(world, random);
        ...
        LOTREntityNumenorFarmer farmer = new LOTREntityNumenorFarmer(world);
        this.spawnNPCAndSetHome(farmer, world, 0, 6, 8, 16);
        int farmhands = 1 + random.nextInt(3);
        for(int l = 0; l < farmhands; ++l) {
            LOTREntityNumenorFarmhand farmhand = new LOTREntityNumenorFarmhand(world);
