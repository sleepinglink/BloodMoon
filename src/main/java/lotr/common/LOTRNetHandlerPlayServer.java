    ...
    public void processMountControl(LOTRPacketMountControl packet) {
        ...
        if(mount != null && mount != this.playerEntity && mount.riddenByEntity == this.playerEntity && LOTRMountFunctions.isMountControllable(mount)) {
            ...
            if(!(!(distSq - speedSq > 150.0) || server.isSinglePlayer() && server.getServerOwner().equals(this.playerEntity.getCommandSenderName()))) {
                FMLLog.warning(mount.getCommandSenderName() + " (mount of " + this.playerEntity.getCommandSenderName() + ") moved too quickly! " + (distSq - speedSq), new Object[0]);
            ...
            if(distSq > 10.0) {
                ...
                FMLLog.warning(mount.getCommandSenderName() + " (mount of " + this.playerEntity.getCommandSenderName() + ") moved wrongly! " + dx + ", " + dy + ", " + dz, new Object[0]);
            }
            ...
            if(!server.isFlightAllowed() && !world.checkBlockCollision(flyCheckBox)) {
                if(movedY >= -0.03125) {
                    ...
                    if(this.floatingMountTick > 80) {
                        FMLLog.warning(this.playerEntity.getCommandSenderName() + " was kicked for floating too long on mount " + mount.getCommandSenderName() + "!", new Object[0]);
