...
public class LOTRGenLayerNearHaradOasis extends LOTRGenLayer {
    ...
    public LOTRGenLayerNearHaradOasis(long l, LOTRGenLayer layer, LOTRDimension dim) {
        super(l);
    ...
    public int[] getInts(World world, int i, int k, int xSize, int zSize) {
        ...
        for(int k1 = 0; k1 < zSize; ++k1) {
            for(int i1 = 0; i1 < xSize; ++i1) {
                ...
                if(biome instanceof LOTRBiomeGenNearHarad && this.nextInt(200) == 0) {
                    boolean surrounded = true;
                    for(int i2 = -1; i2 <= 1; ++i2) {
                        for(int k2 = -1; k2 <= 1; ++k2) {
                            int adjBiomeID = biomes[i1 + 1 + i2 + (k1 + 1 + k2) * (xSize + 2)];
                            LOTRBiome adjBiome = this.dimension.biomeList[adjBiomeID];
                            if(adjBiome instanceof LOTRBiomeGenNearHarad) continue;
                            surrounded = false;
                        }
                    }
                    if(surrounded) {
