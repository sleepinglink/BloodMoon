    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int side, float f, float f1, float f2) {
        ...
        if(entityplayer.canPlayerEdit(i, j, k, side, itemstack) && entityplayer.canPlayerEdit(i + b0, j, k + b1, side, itemstack)) {
            if(world.isAirBlock(i, j, k) && world.isAirBlock(i + b0, j, k + b1) && world.getBlock(i, j - 1, k).isSideSolid(world, i, j - 1, k, ForgeDirection.UP) && world.getBlock(i + b0, j - 1, k + b1).isSideSolid(world, i + b0, j - 1, k + b1, ForgeDirection.UP)) {
                world.setBlock(i, j, k, this.theBedBlock, i1, 3);
                if(world.getBlock(i, j, k) == this.theBedBlock) {
                    world.setBlock(i + b0, j, k + b1, this.theBedBlock, i1 + 8, 3);
                }
                world.playSoundEffect(i + 0.5, j + 0.5, k + 0.5, this.theBedBlock.stepSound.func_150496_b(), (this.theBedBlock.stepSound.getVolume() + 1.0f) / 2.0f, this.theBedBlock.stepSound.getPitch() * 0.8f);
                --itemstack.stackSize;
                return true;
            }
            return false;
