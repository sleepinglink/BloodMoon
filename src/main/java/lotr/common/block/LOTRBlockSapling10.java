...
public class LOTRBlockSapling10 extends LOTRBlockSaplingBase {
    public LOTRBlockSapling10() {
        this.setSaplingNames("boarishnik", "jacaranda");
    }
    ...
    public void growTree(World world, int i, int j, int k, Random random) {
        ...
        int zOffset = 0;
        if(meta == 0) {
            treeGen = LOTRTreeType.BOARISHNIK.create(true, random);
        }
        if(meta == 1) {
            int[] tree2x2;
            int[] tree4x4;
            ...
            if(tree5x5 != null) {
                treeGen = LOTRTreeType.JACARANDA.create(true, random);
                trunkNeg = 2;
                trunkPos = 2;
                xOffset = tree5x5[0];
                zOffset = tree5x5[1];
