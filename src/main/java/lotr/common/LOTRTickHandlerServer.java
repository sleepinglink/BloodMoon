    ...
    public void onWorldTick(TickEvent.WorldTickEvent event) {
        ...
        if(event.phase == TickEvent.Phase.START && world == DimensionManager.getWorld(0)) {
            ...
            for(WorldServer dimWorld : MinecraftServer.getServer().worldServers) {
                ...
                FMLLog.info("LOTR: Successfully replaced world info in %s", LOTRDimension.getCurrentDimension(dimWorld).dimensionName);
            }
        ...
        if(event.phase == TickEvent.Phase.END) {
                        ...
                        if(this.fireworkDisplay > 0) {
                            ...
                            if(world.rand.nextInt(50) == 0) {
                                ...
                                for(int l = 0; l < launches; ++l) {
                                    ...
                                    if(!world.getBlock(i, (j = world.getHeightValue(i, k = MathHelper.floor_double(entityplayer.posZ) + MathHelper.getRandomIntegerInRange(world.rand, -range, range))) - 1, k).isNormalCube()) continue;
            ...
            if(world == DimensionManager.getWorld(LOTRDimension.SHADOW.dimensionID) && !world.playerEntities.isEmpty() && LOTRMod.canSpawnMobs(world)) {
                LOTRSpawnerNPCs.performSpawning(world);
            }
            ...
            if(event.phase == TickEvent.Phase.END) {
                ...
                if(entityplayer.dimension == 0 || entityplayer.dimension == LOTRDimension.MIDDLE_EARTH.dimensionID) {
                    ...
                    for(Object obj : items) {
                        ...
                        if((LOTRLevelData.getData(entityplayer).getAlignment(LOTRFaction.GALADHRIM) >= 1.0f || LOTRLevelData.getData(entityplayer).getAlignment(LOTRFaction.HIGH_ELF) >= 1.0f) && (itemstack.getItem() == Item.getItemFromBlock(LOTRMod.elanor) || itemstack.getItem() == Item.getItemFromBlock(LOTRMod.niphredil))) {
