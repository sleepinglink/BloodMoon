                ...
                for(ChunkCoordIntPair chunkCoords : shuffled) {
                    ...
                    block4: for(int l = 0; l < groups; ++l) {
                        ...
                        for(int a = 0; a < attempts; ++a) {
                            ...
                            float distSq;
                            EntityLiving entity;
                            if(!world.blockExists(i1 += world.rand.nextInt(rangeP1) - world.rand.nextInt(rangeP1), j1 += world.rand.nextInt(yRangeP1) - world.rand.nextInt(yRangeP1), k1 += world.rand.nextInt(rangeP1) - world.rand.nextInt(rangeP1)) || !LOTRSpawnerNPCs.canNPCSpawnAtLocation(world, i1, j1, k1) || world.getClosestPlayer(f = i1 + 0.5f, f1 = j1, f2 = k1 + 0.5f, 24.0) != null || !((distSq = (f3 = f - spawnPoint.posX) * f3 + (f4 = f1 - spawnPoint.posY) * f4 + (f5 = f2 - spawnPoint.posZ) * f5) >= 576.0f)) continue;
                            try {
