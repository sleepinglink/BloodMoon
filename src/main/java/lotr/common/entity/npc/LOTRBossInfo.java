...
public class LOTRBossInfo {
    ...
    public void onUpdate() {
        ...
        if(!this.theNPC.worldObj.isRemote) {
            ...
            for(Map.Entry<UUID, Pair<Integer, Float>> entry : this.playerHurtTimes.entrySet()) {
                ...
                float damage = entry.getValue().getRight().floatValue();
                if(--time <= 0) continue;
                playerHurtTimes_new.put(player, Pair.of(time, Float.valueOf(damage)));
            }
    ...
    public void onHurt(DamageSource damagesource, float f) {
        if(!this.theNPC.worldObj.isRemote) {
            ...
            if((playerSource = LOTRMod.playerSourceOfDamage(damagesource)) != null) {
                ...
                if(this.playerHurtTimes.containsKey(player)) {
                    ...
                    totalDamage += pair.getRight().floatValue();
                }
                this.playerHurtTimes.put(player, Pair.of(time, Float.valueOf(totalDamage)));
            }
        }
    }
    ...
    public void onDeath(DamageSource damagesource) {
        ...
        if(!this.theNPC.worldObj.isRemote) {
            for(Map.Entry<UUID, Pair<Integer, Float>> e : this.playerHurtTimes.entrySet()) {
                ...
                float damage = pair.getRight().floatValue();
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if(data != null) {
            ...
            for(int i = 0; i < playerHurtTags.tagCount(); ++i) {
                ...
                this.playerHurtTimes.put(player, Pair.of(time, Float.valueOf(damage)));
            }
