...
public class LOTRGuiRedBook extends LOTRGuiScreenBase {
                ...
                if(completed) {
                    ...
                    if(quest.anyRewardsGiven()) {
                        ...
                        if(quest.alignmentRewarded != 0.0f) {
                            ...
                            String rewardAlign = StatCollector.translateToLocalFormatted("lotr.gui.redBook.mq.diary.reward.align", alignS, factionName);
    ...
    private void setupScrollBar(int i, int j) {
        ...
        if(this.isScrolling) {
            this.currentScroll = (j - (this.guiTop + this.scrollBarY) - this.scrollWidgetHeight / 2.0f) / ((float) this.scrollBarHeight - (float) this.scrollWidgetHeight);
