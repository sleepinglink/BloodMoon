...
public class LOTRReflectionClient {
    ...
    public static void setCameraRoll(EntityRenderer renderer, float roll) {
        try {
            ObfuscationReflectionHelper.setPrivateValue(EntityRenderer.class, renderer, Float.valueOf(roll), "camRoll", "field_78495_O");
        }
        catch(Exception e) {
    ...
    public static float getCameraRoll(EntityRenderer renderer) {
        try {
            return ((Float) ObfuscationReflectionHelper.getPrivateValue(EntityRenderer.class, renderer, "camRoll", "field_78495_O")).floatValue();
        }
        catch(Exception e) {
            LOTRReflection.logFailure(e);
            return 0.0f;
    ...
    public static void setHandFOV(EntityRenderer renderer, float fov) {
        try {
            ObfuscationReflectionHelper.setPrivateValue(EntityRenderer.class, renderer, Float.valueOf(fov), "fovModifierHand", "field_78507_R");
        }
        catch(Exception e) {
            LOTRReflection.logFailure(e);
        }
    ...
    public static float getHandFOV(EntityRenderer renderer) {
        try {
            return ((Float) ObfuscationReflectionHelper.getPrivateValue(EntityRenderer.class, renderer, "fovModifierHand", "field_78507_R")).floatValue();
        }
        catch(Exception e) {
            LOTRReflection.logFailure(e);
            return 0.0f;
    ...
    public static float getFOVModifier(EntityRenderer renderer, float tick, boolean flag) {
        try {
            ...
            return ((Float) method.invoke(renderer, Float.valueOf(tick), flag)).floatValue();
        }
        catch(Exception e) {
            LOTRReflection.logFailure(e);
            return 0.0f;
    ...
    public static void setHighlightedItemTicks(GuiIngame gui, int ticks) {
        try {
            ObfuscationReflectionHelper.setPrivateValue(GuiIngame.class, gui, Integer.valueOf(ticks), "remainingHighlightTicks", "field_92017_k");
        }
        catch(Exception e) {
            LOTRReflection.logFailure(e);
        }
