...
public class LOTRBiomeVariantStorage {
    ...
    private static Map<ChunkCoordIntPair, byte[]> getDimensionChunkMap(World world) {
        ...
        Map<ChunkCoordIntPair, byte[]> map = sourcemap.get(dim = LOTRDimension.getCurrentDimension(world));
        if(map == null) {
    ...
    public static void clearAllVariants(World world) {
        ...
        FMLLog.info("Unloading LOTR biome variants in %s", LOTRDimension.getCurrentDimension(world).dimensionName);
    }
