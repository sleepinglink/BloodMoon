    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
                ...
                for(k1 = k - trunkCircleWidth - 1; k1 <= k + trunkCircleWidth + 1 && flag; ++k1) {
                    ...
                    for(j1 = j; j1 <= j + 1 + height; ++j1) {
                        if(j1 >= 0 && j1 < 256) {
                            ...
                            if(this.isReplaceable(world, i1, j1, k1) || block.isReplaceable(world, i1, j1, k1)) continue;
                            flag = false;
                            continue;
                        }
                        flag = false;
        ...
        for(j12 = 0; j12 < height; ++j12) {
            for(int i12 = i - trunkCircleWidth - 1; i12 <= i + trunkCircleWidth + 1; ++i12) {
                for(int k12 = k - trunkCircleWidth - 1; k12 <= k + trunkCircleWidth + 1; ++k12) {
                    ...
                    this.setBlockAndNotifyAdequately(world, i12, j + j12, k12, this.woodBlock, this.woodMeta);
                }
            }
            if(j12 % xSlope == 0) {
                if(xSlope > 0) {
        ...
        for(j12 = j + height - 1; j12 > j + (int) (height * 0.75f); --j12) {
            ...
            for(int l = 0; l < branches; ++l) {
                ...
                for(int l1 = trunkCircleWidth; l1 < trunkCircleWidth + length && this.isReplaceable(world, i13 = i + (int) (1.5f + MathHelper.cos(angle) * l1), j2 = j12 - 3 + l1 / 2, k13 = k + (int) (1.5f + MathHelper.sin(angle) * l1)); ++l1) {
