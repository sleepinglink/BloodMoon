    ...
    public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
        if(super.generateWithSetRotation(world, random, i, j, k, rotation)) {
            ...
            for(int l = 0; l < dummies; ++l) {
                ...
                float ang;
                int rot;
                for(int att = 0; att < 8 && !this.generateSubstructureWithRestrictionFlag(new LOTRWorldGenMoredainMercDummy(this.notifyChanges), world, random, i1 = (int) ((r = MathHelper.getRandomIntegerInRange(random, 8, 15)) * MathHelper.cos(ang = random.nextFloat() * 3.1415927f * 2.0f)), j1 = this.getTopBlock(world, i1, k1 = (int) (r * MathHelper.sin(ang))), k1, rot = random.nextInt(4), true); ++att) {
                }
            }
            return true;
        }
