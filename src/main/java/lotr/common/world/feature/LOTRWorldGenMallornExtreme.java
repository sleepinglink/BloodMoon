            ...
            for(i1 = i - trunkWidth; i1 <= i + trunkWidth; ++i1) {
                for(k1 = k - trunkWidth; k1 <= k + trunkWidth; ++k1) {
                    ...
                    for(int j1 = 0; j1 < height; ++j1) {
                        ...
                        if(!block.isReplaceable(world, i1, j + j1, k1) && !block.isLeaves(world, i1, j + j1, k1)) continue;
            ...
            if(trunkWidth > 0) {
                for(int j1 = j + (int) (height * BOUGH_BASE_HEIGHT_MIN); j1 > j + (int) (height * 0.67f); j1 -= 1 + random.nextInt(3)) {
                    ...
                    for(int b = 0; b < branches; ++b) {
                        ...
                        for(int l = 0; l < length && this.isReplaceable(world, i13 = i + (int) (1.5f + MathHelper.cos(branchAngle) * l), j2 = j1 - 3 + l / 2, k13 = k + (int) (1.5f + MathHelper.sin(branchAngle) * l)); ++l) {
        ...
        for(int i1 = i - leafRange; i1 <= i + leafRange; ++i1) {
            for(int j1 = j - leafRange; j1 <= j + leafRange; ++j1) {
                for(int k1 = k - leafRange; k1 <= k + leafRange; ++k1) {
                    ...
                    if(dist >= leafRangeSqLess && (dist >= leafRangeSq || random.nextInt(3) != 0) || !(block = world.getBlock(i1, j1, k1)).isReplaceable(world, i1, j1, k1) && !block.isLeaves(world, i1, j1, k1)) continue;
    ...
    private void spawnLeafLayer(World world, Random random, int i, int j, int k, int leafRange) {
        ...
        for(int i1 = i - leafRange; i1 <= i + leafRange; ++i1) {
            for(int k1 = k - leafRange; k1 <= k + leafRange; ++k1) {
                ...
                if(dist > leafRangeSq || !(block = world.getBlock(i1, j, k1)).isReplaceable(world, i1, j, k1) && !block.isLeaves(world, i1, j, k1)) continue;
