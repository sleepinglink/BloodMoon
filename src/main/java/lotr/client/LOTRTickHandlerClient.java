...
public class LOTRTickHandlerClient {
                ...
                block76: {
                    block77: {
                        ...
                        if(LOTRConfig.enableSunFlare && world.provider instanceof LOTRWorldProvider && !world.provider.hasNoSky) {
                            ...
                            float sunPitch = celestialAngle = world.getCelestialAngle(renderTick) * 360.0f - 90.0f;
                            float sunYaw = 90.0f;
    ...
    public void onRenderTick(TickEvent.RenderTickEvent event) {
        ...
        if(event.phase == TickEvent.Phase.START) {
            ...
            if(this.cancelItemHighlight && (highlightTicks = LOTRReflectionClient.getHighlightedItemTicks(guiIngame = minecraft.ingameGUI)) > 0) {
    ...
    private void updatePlayerInPortal(EntityPlayer entityplayer, HashMap players, Block portalBlock) {
        if((entityplayer.dimension == 0 || entityplayer.dimension == LOTRDimension.MIDDLE_EARTH.dimensionID) && players.containsKey(entityplayer)) {
            boolean inPortal;
            boolean bl = inPortal = entityplayer.worldObj.getBlock(MathHelper.floor_double(entityplayer.posX), MathHelper.floor_double(entityplayer.boundingBox.minY), MathHelper.floor_double(entityplayer.posZ)) == portalBlock;
            if(inPortal) {
    ...
    private void spawnEnvironmentFX(EntityPlayer entityplayer, World world) {
        ...
        for(int l = 0; l < 1000; ++l) {
            ...
            if(block.getMaterial() != Material.water || meta == 0 || (below = world.getBlock(i1, j1 - 1, k1)).getMaterial() != Material.water) continue;
    ...
    public void onPreRenderGameOverlay(RenderGameOverlayEvent.Pre event) {
        ...
        if(world != null && entityplayer != null) {
            ...
            if(event.type == RenderGameOverlayEvent.ElementType.ALL) {
                ...
                if(itemstack != null && !itemstack.hasDisplayName() && !(enchants = LOTREnchantmentHelper.getEnchantList(itemstack)).isEmpty()) {
            ...
            if(event.type == RenderGameOverlayEvent.ElementType.HELMET) {
                ...
                if(this.frostTick > 0) {
                    this.renderOverlay(null, this.frostTick / 80.0f * 0.9f, mc, frostOverlay);
                }
                if(this.burnTick > 0) {
    ...
    public void getItemTooltip(ItemTooltipEvent event) {
        ...
        if(LOTRWeaponStats.isMeleeWeapon(itemstack)) {
            ...
            if(dmgIndex >= 0) {
                ...
                meleeDamage = LOTRWeaponStats.getMeleeDamageBonus(itemstack);
        ...
        if (LOTRWeaponStats.isRangedWeapon(itemstack)) {
            tooltip.add("");
            meleeDamage = LOTRWeaponStats.getRangedSpeed(itemstack);
            int pcDamage;         
            if (meleeDamage > 0.0F) {
                if (itemstack.getItem() instanceof LOTRItemThrowingAxe) {
                   tooltip.add(EnumChatFormatting.DARK_GREEN + StatCollector.translateToLocalFormatted("lotr.weaponstat.rangedCooldown", new Object[]{meleeDamage}));
                } else {
                   pcDamage = Math.round(meleeDamage * 100.0F);
                   tooltip.add(EnumChatFormatting.DARK_GREEN + StatCollector.translateToLocalFormatted("lotr.weaponstat.rangedSpeed", new Object[]{pcDamage}));
                }
             }
            int kb;
            float damage;
            if((damage = LOTRWeaponStats.getRangedDamageFactor(itemstack, false)) > 0.0f) {
                pcDamage = Math.round(damage * 100.0f);
        ...
        if(itemstack.getTagCompound() != null && itemstack.getTagCompound().hasKey("LOTROwner")) {
            ...
            owner = StatCollector.translateToLocalFormatted("item.lotr.generic.owner", owner);
            List ownerLines = Minecraft.getMinecraft().fontRenderer.listFormattedStringToWidth(owner, 200);
    ...
    public void onFOVUpdate(FOVUpdateEvent event) {
        ...
        if(entityplayer.isUsingItem()) {
            ...
            else if(item instanceof LOTRItemJavelin) {
                maxDrawTime = ((LOTRItemJavelin) item).getMaxDrawTime();
            }
    ...
    public void onRenderFog(EntityViewRenderEvent.RenderFogEvent event) {
        ...
        if(provider instanceof LOTRWorldProvider) {
            ...
            if(LOTRConfig.newRain && (lotrbiome.getEnableRain() || lotrbiome.getEnableSnow()) && (rain = this.prevRainFactor + (this.rainFactor - this.prevRainFactor) * renderTick) > 0.0f) {
