        ...
        if(j >= 1 && j + height + 1 <= 256) {
            ...
            for(i12 = i; i12 <= i + this.extraTrunk && canGrow; ++i12) {
                for(k1 = k; k1 <= k + this.extraTrunk && canGrow; ++k1) {
                    ...
                    if(below.canSustainPlant(world, i12, j - 1, k1, ForgeDirection.UP, (IPlantable) (Blocks.sapling))) continue;
                    canGrow = false;
                }
            }
            if(canGrow) {
                ...
                for(j1 = leafStart; j1 <= leafTop; ++j1) {
                    ...
                    for(int i13 = i - leafRange; i13 <= i + this.extraTrunk + leafRange; ++i13) {
                        for(int k13 = k - leafRange; k13 <= k + this.extraTrunk + leafRange; ++k13) {
                            ...
                            if((dCh = i2 + k2) > 4 || (i2 >= leafRange || k2 >= leafRange) && random.nextInt(3) == 0 || !(block = world.getBlock(i13, j1, k13)).isReplaceable(world, i13, j1, k13) && !block.isLeaves(world, i13, j1, k13)) continue;
