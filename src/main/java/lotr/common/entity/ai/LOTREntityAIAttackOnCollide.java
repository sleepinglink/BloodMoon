...
public class LOTREntityAIAttackOnCollide extends EntityAIBase {
    ...
    public void updateTask() {
        ...
        if((weapon = this.theOwner.getHeldItem()) != null && weapon.getItem() instanceof LOTRItemJavelin && this.attackTick <= 0 && this.theOwner instanceof LOTREntityNPC) {
            ...
            if(spearBackup != null) {
                ...
                if(d > 5.0 && d < range * 0.75) {
                    LOTREntityJavelin spear = new LOTREntityJavelin(this.worldObj, this.theOwner, this.attackTarget, weapon.copy(), 0.75f + (float) d * 0.025f, 0.5f);
