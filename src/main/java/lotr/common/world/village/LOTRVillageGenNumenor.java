...
public class LOTRVillageGenNumenor extends LOTRVillageGen {
    ...
    private LOTRWorldGenNumenorStructure.GondorFiefdom villageFief;

    public LOTRVillageGenNumenor(LOTRBiome biome, LOTRWorldGenNumenorStructure.GondorFiefdom fief, float f) {
        super(biome);
        this.gridScale = 16;
    ...
    public class Instance extends LOTRVillageGen.AbstractInstance {
        ...
        protected void addStructure(LOTRWorldGenStructureBase2 structure, int x, int z, int r, boolean force) {
            ...
            if(structure instanceof LOTRWorldGenNumenorStructure) {
                ((LOTRWorldGenNumenorStructure) structure).strFief = LOTRVillageGenNumenor.this.villageFief;
            }
        }
        ...
        private void setupVillage(Random random) {
            ...
            this.addStructure(new LOTRWorldGenNPCRespawner(false) {
                ...
                public void setupRespawner(LOTREntityNPCRespawner spawner) {
                    spawner.setSpawnClass(LOTREntityNumenor.class);
            ...
            this.addStructure(new LOTRWorldGenNPCRespawner(false) {
                ...
                @Override
                public void setupRespawner(LOTREntityNPCRespawner spawner) {
                    spawner.setSpawnClass(LOTRVillageGenNumenor.this.villageFief.getLevyClasses()[0]);
                    spawner.setCheckRanges(40, -12, 12, 16);
                    spawner.setSpawnRanges(20, -6, 6, 64);
                    spawner.setBlockEnemySpawnRange(60);
                }
            }, 0, 0, 0);
            this.addStructure(new LOTRWorldGenNumenorCottage(false), -21, 0, 1);
            this.addStructure(new LOTRWorldGenNumenorCottage(false), 0, -21, 2);
            this.addStructure(new LOTRWorldGenNumenorCottage(false), 21, 0, 3);
            this.addStructure(new LOTRWorldGenNumenorTavern(false), 0, 21, 0);
            if(random.nextBoolean()) {
                if(random.nextInt(3) == 0) {
                    this.addStructure(LOTRWorldGenNumenorMarketStall.getRandomStall(random, false), -9, -12, 1);
                }
                if(random.nextInt(3) == 0) {
                    this.addStructure(LOTRWorldGenNumenorMarketStall.getRandomStall(random, false), 9, -12, 3);
                }
                if(random.nextInt(3) == 0) {
                    this.addStructure(LOTRWorldGenNumenorMarketStall.getRandomStall(random, false), -9, 12, 1);
                }
        ...
        private LOTRWorldGenStructureBase2 getRandomHouse(Random random) {
            if(random.nextInt(5) == 0) {
                ...
                if(i == 0) {
                    return new LOTRWorldGenNumenorStables(false);
                }
                if(i == 1) {
                    return new LOTRWorldGenNumenorSmithy(false);
                }
                if(i == 2) {
                    return new LOTRWorldGenNumenorBarn(false);
                }
            }
            return new LOTRWorldGenNumenorHouse(false);
        }

        private LOTRWorldGenStructureBase2 getRandomFarm(Random random) {
            if(random.nextBoolean()) {
                if(random.nextBoolean()) {
                    return new LOTRWorldGenNumenorVillageFarm.Animals(false);
                }
                return new LOTRWorldGenNumenorVillageFarm.Crops(false);
            }
            return new LOTRWorldGenNumenorVillageFarm.Tree(false);
        }
        ...
        private void setupTown(Random random) {
            ...
            for(int i1 : new int[] {-40, 40}) {
                ...
                for(int i = 0; i < n; ++i) {
                    ...
                    this.addStructure(new LOTRWorldGenNPCRespawner(false) {
                        ...
                        @Override
                        public void setupRespawner(LOTREntityNPCRespawner spawner) {
                            spawner.setSpawnClasses(LOTRVillageGenNumenor.this.villageFief.getLevyClasses()[0], LOTRVillageGenNumenor.this.villageFief.getLevyClasses()[1]);
                            spawner.setCheckRanges(40, -12, 12, 16);
                            spawner.setSpawnRanges(20, -6, 6, 64);
                            spawner.setBlockEnemySpawnRange(60);
                        }
            ...
            for(int k1 = -1; k1 <= 1; ++k1) {
                int k2 = k1 * stallPos;
                if(random.nextInt(3) != 0) {
                    this.addStructure(LOTRWorldGenNumenorMarketStall.getRandomStall(random, false), -stallPos + 3, k2, 1, true);
                }
                if(random.nextInt(3) == 0) continue;
                this.addStructure(LOTRWorldGenNumenorMarketStall.getRandomStall(random, false), stallPos - 3, k2, 3, true);
            }
            if(random.nextInt(3) != 0) {
                this.addStructure(LOTRWorldGenNumenorMarketStall.getRandomStall(random, false), 0, stallPos - 3, 0, true);
            }
            if(random.nextInt(3) != 0) {
                this.addStructure(LOTRWorldGenNumenorMarketStall.getRandomStall(random, false), 0, -stallPos + 3, 2, true);
            }
            int flowerX = 12;
            int flowerZ = 18;
            ...
            for(int k1 = -1; k1 <= 1; ++k1) {
                int houseZ = k1 * 12;
                if(k1 == 1) {
                    this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), -houseX, houseZ, 1, true);
                    this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), houseX, houseZ, 3, true);
                }
                if(k1 == 0) continue;
                this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), houseZ, houseX, 0, true);
                this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), houseZ, -houseX, 2, true);
            }
            this.addStructure(new LOTRWorldGenNumenorSmithy(false), 0, -26, 2, true);
            this.addStructure(new LOTRWorldGenGondorObelisk(false), 0, 27, 0, true);
            this.addStructure(new LOTRWorldGenNumenorTavern(false), -houseX, -5, 1, true);
            ...
            this.addStructure(new LOTRWorldGenNumenorBath(false), houseX + 2, -6, 3, true);
            ...
            for(int k1 = -4; k1 <= 4; ++k1) {
                ...
                if(treepiece) {
                    this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), -houseX - 2, houseZ, 1, true);
                    this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), houseX + 2, houseZ, 3, true);
                }
                else {
                    this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), -houseX, houseZ, 1, true);
                    this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), houseX, houseZ, 3, true);
                ...
                if(treepiece) {
                    this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), houseZ, -houseX - 2, 2, true);
                }
                else {
                    this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), houseZ, -houseX, 2, true);
                }
                if(!(Math.abs(k1) < 2 || outerTavern && k1 > 2)) {
                    if(treepiece) {
                        this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), houseZ, houseX + 2, 0, true);
                    }
                    else {
                        this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), houseZ, houseX, 0, true);
                    }
            ...
            if(outerTavern) {
                this.addStructure(new LOTRWorldGenNumenorTavern(false), 44, houseX, 0, true);
            }
            int gardenX = 42;
            int gardenZ = 48;
            this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), -gardenX, -gardenZ, 1, true);
            this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), -gardenX, gardenZ, 1, true);
            this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), gardenX, -gardenZ, 3, true);
            this.addStructure(new LOTRWorldGenNumenorVillageFarm.Tree(false), gardenX, gardenZ, 3, true);
            int obeliskX = 62;
            int obeliskZ = 66;
            ...
            this.addStructure(new LOTRWorldGenNumenorGatehouse(false).setSignText(this.villageName), 0, 84, 2, true);
        ...
        private void setupFortVillage(Random random) {
            ...
            for(int i1 : new int[] {-20, 20}) {
                for(int k1 : new int[] {-20, 20}) {
                    this.addStructure(new LOTRWorldGenNPCRespawner(false) {
                        ...
                        @Override
                        public void setupRespawner(LOTREntityNPCRespawner spawner) {
                            spawner.setSpawnClasses(LOTRVillageGenNumenor.this.villageFief.getSoldierClasses()[0], LOTRVillageGenNumenor.this.villageFief.getSoldierClasses()[1]);
            ...
            this.addStructure(new LOTRWorldGenNumenorStables(false), -24, 2, 0);
            this.addStructure(new LOTRWorldGenNumenorStables(false), -24, -2, 2);
            this.addStructure(new LOTRWorldGenNumenorSmithy(false), 24, 1, 0);
            this.addStructure(new LOTRWorldGenNumenorSmithy(false), 24, -1, 2);
            this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), -3, -25, 1);
            this.addStructure(new LOTRWorldGenNumenorStoneHouse(false), 3, -25, 3);
            this.addStructure(new LOTRWorldGenNumenorVillageFarm.Crops(false), -18, -21, 1);
            this.addStructure(new LOTRWorldGenNumenorVillageFarm.Crops(false), 18, -21, 3);
        ...
        private LOTRWorldGenStructureBase2 getVillageFortress() {
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.GONDOR) {
                return new LOTRWorldGenGondorFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.NUMENOR) {
                return new LOTRWorldGenNumenorFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.LOSSARNACH) {
                return new LOTRWorldGenLossarnachFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.LEBENNIN) {
                return new LOTRWorldGenLebenninFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.PELARGIR) {
                return new LOTRWorldGenPelargirFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.PINNATH_GELIN) {
                return new LOTRWorldGenPinnathGelinFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.BLACKROOT_VALE) {
                return new LOTRWorldGenBlackrootFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.LAMEDON) {
                return new LOTRWorldGenLamedonFortress(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.DOL_AMROTH) {
        ...
        private LOTRWorldGenStructureBase2 getVillageWatchtower() {
            ...
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.NUMENOR) {
                return new LOTRWorldGenNumenorWatchtower(false);
            }
            if(LOTRVillageGenNumenor.this.villageFief == LOTRWorldGenNumenorStructure.GondorFiefdom.LOSSARNACH) {
        ...
        protected LOTRRoadType getPath(Random random, int i, int k) {
            ...
            if(this.villageType == VillageType.FORT) {
                if(i1 <= 1 && (k >= 13 || k <= -12) && k1 <= 36) {
                    return LOTRVillageGenNumenor.this.villageBiome.getRoadBlock();
                }
                if(k1 <= 1 && i1 >= 12 && i1 <= 36) {
                    return LOTRVillageGenNumenor.this.villageBiome.getRoadBlock();
                }
