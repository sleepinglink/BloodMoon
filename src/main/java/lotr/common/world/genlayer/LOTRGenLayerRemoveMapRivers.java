            ...
            for(int i1 = 0; i1 < xSize; ++i1) {
                ...
                if(biomeID == LOTRBiome.river.biomeID) {
                    ...
                    for(int range = 1; range <= maxRange; ++range) {
                        ...
                        int subBiomeID;
                        int count;
                        HashMap<Integer, Integer> viableBiomes = new HashMap<Integer, Integer>();
                        ...
                        for(int k2 = k1 - range; k2 <= k1 + range; ++k2) {
                            for(int i2 = i1 - range; i2 <= i1 + range; ++i2) {
                                int subIndex;
                                LOTRBiome subBiome;
                                if(Math.abs(i2 - i1) != range && Math.abs(k2 - k1) != range || (subBiome = this.dimension.biomeList[subBiomeID = biomes[subIndex = i2 + maxRange + (k2 + maxRange) * (xSize + maxRange * 2)]]) == LOTRBiome.river) continue;
