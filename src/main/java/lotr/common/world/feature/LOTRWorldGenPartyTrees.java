            ...
            if(j >= 1 && j + height + 1 <= 256) {
                ...
                for(i1 = i - trunkWidth; i1 <= i + trunkWidth && flag; ++i1) {
                    for(k1 = k - trunkWidth; k1 <= k + trunkWidth && flag; ++k1) {
                        ...
                        if(block.canSustainPlant(world, i1, j - 1, k1, ForgeDirection.UP, (IPlantable) (Blocks.sapling))) continue;
                        flag = false;
                    }
                }
                if(!flag) {
                    ...
                    for(int i3 = i2 - leafRange; i3 <= i2 + leafRange; ++i3) {
                        for(int j3 = j2 - leafRange; j3 <= j2 + leafRange; ++j3) {
                            for(int k3 = k2 - leafRange; k3 <= k2 + leafRange; ++k3) {
                                ...
                                if(dist >= (leafRange - 1) * (leafRange - 1) && (dist >= leafRange * leafRange || random.nextInt(3) == 0) || !(block2 = world.getBlock(i3, j3, k3)).isReplaceable(world, i3, j3, k3) && !block2.isLeaves(world, i3, j3, k3)) continue;
