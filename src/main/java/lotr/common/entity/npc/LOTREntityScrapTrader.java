...
public class LOTREntityScrapTrader extends LOTREntityMan implements LOTRTravellingTrader, LOTRTradeable.Smith {
    ...
    public boolean interact(EntityPlayer entityplayer) {
        ...
        if(flag && !this.worldObj.isRemote && LOTRDimension.getCurrentDimension(this.worldObj) == LOTRDimension.SHADOW && this.timeUntilFadeOut <= 0) {
    ...
    public boolean canBeFreelyTargetedBy(EntityLiving attacker) {
        if(LOTRDimension.getCurrentDimension(this.worldObj) == LOTRDimension.SHADOW) {
            return false;
        }
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float f) {
        if(damagesource.getEntity() != null && LOTRDimension.getCurrentDimension(this.worldObj) == LOTRDimension.SHADOW) {
    ...
    public void onLivingUpdate() {
        ...
        if(!this.worldObj.isRemote && LOTRDimension.getCurrentDimension(this.worldObj) == LOTRDimension.SHADOW) {
    ...
    public boolean canTradeWith(EntityPlayer entityplayer) {
        return this.isFriendly(entityplayer) && LOTRDimension.getCurrentDimension(this.worldObj) != LOTRDimension.SHADOW;
    }
