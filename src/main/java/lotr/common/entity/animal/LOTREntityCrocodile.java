...
public class LOTREntityCrocodile extends EntityMob {
            ...
            for(int i = -8; i <= 8; ++i) {
                for(int j = -8; j <= 8; ++j) {
                    for(int k = -8; k <= 8; ++k) {
                        ...
                        if(!this.worldObj.blockExists(i1, j1 = MathHelper.floor_double(this.posY) + j, k1 = MathHelper.floor_double(this.posZ) + k) || (block = this.worldObj.getBlock(i1, j1, k1)).getMaterial() != Material.water) continue;
                        if(this.posY > 60.0) {
                            return true;
                        }
    ...
    protected boolean isValidLightLevel() {
        ...
        if(this.worldObj.getBiomeGenForCoords(i, k = MathHelper.floor_double(this.posZ)) instanceof LOTRBiomeGenFarHaradSwamp) {
            return true;
        }
